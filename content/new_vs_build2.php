...
$logger->info("new_vs_build2.php UN: " .$_SESSION['username'] . " Role: " . $_SESSION['role'] . " LoggedIn: " . $_SESSION['loggedin']);
if ($_SESSION['loggedin'] != true){
    session_unset();
    session_destroy();
    #file_put_contents("/var/log/chaniqphp.log", "new_vs_build2.php redirection to login page!!\n", FILE_APPEND);
    $logger->info("new_vs_build2.php redirection to login page!!");
...
$logger->info("new_vs_build2.php() - callBack function php has been called");
...
if (isset($_POST['jsonVsData'])){
    ...
    $logger->info("new_vs_build2() phpFile: " . $vsData->PhpFileName);
...
function new_vs_build2($vsData, $logger) {
    #file_put_contents("/var/log/chaniqphp.log", "new_vs_build2() called\n", FILE_APPEND);
    $logger->info("new_vs_build2() called");
    ...
    $logger->info("new_vs_build2() Device IP: " . $vsDevIp . " VS Name: " .$vs_dnsname. " VS Port: " .$vs_port. " VS Env: " .$vs_env. " TCP Profile: " .$vs_tcpprofile.  " VS Redirection: " .$vs_redirect. " VS Persist: " .$vs_persistence. " VS Type: " .$vs_type. " VS iRule: " .$vs_irule. " VS Poolname: " .$vs_poolname);
    
    $cmd = '/usr/bin/python2 /var/www/chaniq/py/new_vs_build2.py '. escapeshellarg($vsDevIp) .' '. escapeshellarg($vs_dnsname) .' '. escapeshellarg($vs_dest) .' '. escapeshellarg($vs_port) .' '. escapeshellarg($vs_desc) .' '. escapeshellarg($vs_env) .' '. escapeshellarg($vs_tcpprofile).' '. escapeshellarg($vs_persistence) .' '. escapeshellarg($vs_redirect) .' '.escapeshellarg($vs_type) .' '.escapeshellarg($vs_httpprofile) .' '.escapeshellarg($vs_sslclient) .' '.escapeshellarg($vs_sslserver) .' '. escapeshellarg($vs_irule) .' '.escapeshellarg($vs_snatpool) .' '.escapeshellarg($vs_policy) .' '.escapeshellarg($vs_poolname);
    
    $output = shell_exec($cmd);
    #error_log(date("y-m-d H:i:s").": After python call -new_vs_build2.php() new_vs_build2() function called!\n", 3, "/var/log/chaniqphp.log");
    $logger->info("After python call -new_vs_build2.php() new_vs_build2() function called!");
