...
$logger->info("new_node_build.php UN: " .$_SESSION['username'] . " Role: " . $_SESSION['role'] . " LoggedIn: " . $_SESSION['loggedin']);
if ($_SESSION['loggedin'] != true){
    session_unset();
    session_destroy();
    #file_put_contents("/var/log/chaniqphp.log", "new_node_build.php redirection to login page!!\n", FILE_APPEND);
    $logger->info("new_node_build.php redirection to login page!!");
...
$logger->info("new_node_build.php() - callBack function php has been called");
...
if (isset($_POST['jsonNodeData'])){
    ...
    $logger->info("new_node_build() phpFile: " . $nodeData->PhpFileName);
    
    // Call the fuction new_node_build()
    echo ($nodeData->PhpFileName)($nodeData, $logger);
}
else{
...
function new_node_build($nodeData, $logger) {
    ...
    $logger->info("After python call -new_node_build.php() new_node_build() function called!");
