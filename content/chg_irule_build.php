...
require_once(__DIR__ . '/../utility/chaniqLogger.php');
    ...
    session_start();
    #file_put_contents("/var/log/chaniqphp.log", "chg_irule_build.php UN: " .$_SESSION['username'] . " Role: " . $_SESSION['role'] . " LoggedIn: " . $_SESSION['loggedin'] . "\n", FILE_APPEND);
    $logger->info("chg_irule_build.php UN: " .$_SESSION['username'] . " Role: " . $_SESSION['role'] . " LoggedIn: " . $_SESSION['loggedin']);
    if ($_SESSION['loggedin'] != true){
        session_unset();
        session_destroy();
        #file_put_contents("/var/log/chaniqphp.log", "chg_irule_build.php redirection to login page!!\n", FILE_APPEND);
        $logger->info("chg_irule_build.php redirection to login page!!");
    ...
    function new_irule_build($irData, $logger) {
        ...
        $cmd = '/usr/bin/python2 /var/www/chaniq/py/new_irule_build.py '. escapeshellarg($irDevIp) .' '. escapeshellarg($irDgName) .' '. escapeshellarg($irEnv) .' '. escapeshellarg($irType) .' '. escapeshellarg($irCode) .' '. escapeshellarg($irDgType) .' '. escapeshellarg($irDgData);
