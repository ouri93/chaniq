...
def chg_pool_build(active_ltm, p_name, p_part, p_mon, p_LB, p_priGroup, p_priLessthan, pm_names, pm_ips, pm_ratios, pm_mons, pm_priGroups):
    ...
    logging.info("Pool Configuration Modification process has been initiated. Pool Name: " + p_name)
    ...
    try:
        ...
        logging.info("Exception during Pool configuration modification process")
        strReturn[str(idx)] = "Exception error: " + str(e)
        idx += 1
        logging.info("Pool configuration modification exception: " + str(e))
        return json.dumps(strReturn)

    logging.info("Pool configuration modification has been successfully completed.")
    ...
    logging.info("Converted member names: " + strMemberNames)
    try:
        p_loaded = mr.tm.ltm.pools.pool.load(partition=p_part, name=p_name)
        for PMbrs in p_loaded.members_s.get_collection():
            #if membernames.find(PMbrs.name) != -1:
            logging.info("Pool member name from BIG-IP: " + PMbrs.name + "     Pool member name from Input: " + strMemberNames)
            if strMemberNames.find(PMbrs.name) < 0:
                logging.info("Not matching pool memebr name found: " + PMbrs.name)
                pm1 = p_loaded.members_s.members.load(partition=p_part, name=p_name)
                pm1.delete()
                logging.info("Pool member (" + p_name + ") has been successfully deleted")
    except Exception as e:
        logging.info("Exception during Pool member loading and deletion process")
        strReturn[str(idx)] = "Exception error: " + str(e)
        idx += 1
        logging.info("Pool member loading and deletion process exception: " + str(e))
        return json.dumps(strReturn)
    
    logging.info("Pool member deletion has been successfully completed.")
    ...
    try:
        for i in range(len(membernames)-1):
            logging.info("Index: " + str(i) + "  Num of Pool members: " + str(len(membernames)-1) + "  First pool member name: " + membernames[i])
            ...
            if p_loaded.members_s.members.exists(partition=p_part, name=membernames[i]):
                ...
                logging.info("PM Ratio: " + str(pm1.ratio) + "  PM Mon: " + pm1.monitor + "  PM PriGroup: " + str(pm1.priorityGroup) + " ")
        ...
        logging.info("Exception during Pool member addition and modification process")
        strReturn[str(idx)] = "Exception error: " + str(e)
        idx += 1
        logging.info("Pool member addition and modification process exception: " + str(e))
        return json.dumps(strReturn)

    logging.info("Pool member addition and modification have been successfully completed.")    
